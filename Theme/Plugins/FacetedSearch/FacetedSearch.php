...
class FacetedSearch extends Module implements WidgetInterface
{
    ...
    public function install()
    {
        ...
        if ($installed) {
            ...
            $products_count = Database::getInstance()->getValue('SELECT COUNT(*) FROM `'._DB_PREFIX_.'product`');
    ...
    public function hookProductSearchProvider($params)
    {
        ...
        if ($query->getIdCategory()) {
            return new FacetedSearchProductSearchProvider($this);
        } else {
            return null;
        }
    }
    ...
    public function uninstall()
    {
        ...
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_price_index');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_indexable_attribute_group');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_indexable_feature');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_indexable_attribute_lang_value');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_indexable_attribute_group_lang_value');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_indexable_feature_lang_value');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_indexable_feature_value_lang_value');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_category');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_filter');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_filter_shop');
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'layered_product_attribute');
    ...
    {
        $values = array();
        $is_indexable = Database::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            'SELECT `indexable`
        ...
        if ($result = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
    ...
    {
        $layered_filter_list = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
    ...
    public function indexAttribute($id_product = null)
    {
        if (is_null($id_product)) {
            Database::getInstance()->execute('TRUNCATE '._DB_PREFIX_.'layered_product_attribute');
        } else {
            Database::getInstance()->execute(
    ...
    {
        if ($full) {
            $nb_products = (int) Database::getInstance()->getValue('
    ...
    private static function indexPricesUnbreakable($cursor, $full = false, $smart = false)
    {
        ...
        foreach (Database::getInstance()->executeS($query) as $product) {
    ...
    public static function indexProductPrices($id_product, $smart = true)
    {
        ...
        if (is_null($groups)) {
            $groups = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT id_group FROM `'._DB_PREFIX_.'group_reduction`');
            if (!$groups) {
                $groups = array();
            }
        }
        ...
        foreach ($shop_list as $id_shop) {
            ...
            if ($smart) {
                Database::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'layered_price_index` WHERE `id_product` = '.(int) $id_product.' AND `id_shop` = '.(int) $id_shop);
            }

            if (Configuration::get('PS_LAYERED_FILTER_PRICE_USETAX')) {
                $max_tax_rate = Database::getInstance()->getValue('
            ...
            $product_min_prices = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('SubmitFilter')) {
            if (!Tools::getValue('layered_tpl_name')) {
                ...
                if (Tools::getValue('scope') == 1) {
                    Database::getInstance()->execute('TRUNCATE TABLE '._DB_PREFIX_.'layered_filter');
                    $categories = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                        'SELECT id_category
                ...
                if (!$id_layered_filter) {
                    $id_layered_filter = (int) Database::getInstance()->Insert_ID();
                }
            ...
            $layered_values = Database::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
                'SELECT filters
        ...
        $category_box = array();
        $attribute_groups = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
        ...
        $features = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
        ...
        if (Tools::getValue('add_new_filters_template')) {
            ...
            $template = Database::getInstance()->getRow(
                '
				SELECT *
				FROM `'._DB_PREFIX_.'layered_filter`
				WHERE id_layered_filter = '.(int) Tools::getValue('id_layered_filter')
            ...
            $this->context->smarty->assign(array(
                ...
                'filters_templates' => Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM '._DB_PREFIX_.'layered_filter ORDER BY date_add DESC'),
    ...
    public function getProductByFilters(
        ...
        if (isset($price_filter) && $price_filter) {
            ...
            if (empty($selected_filters['category'])) {
                $all_products_out = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
        ...
        $this->nbr_products = Database::getInstance(_PS_USE_SQL_SLAVE_)->getValue('SELECT COUNT(*) FROM ' . $catFilterRestrictionDerivedTable . ' ps');
        ...
        if ($this->nbr_products == 0) {
            ...
            if (version_compare(_PS_VERSION_, '1.6.1', '>=') === true) {
                $products = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                    SELECT
                        p.*,
    ...
    {
        return Database::getInstance(_PS_USE_SQL_SLAVE_)->query($sql_query);
    }
    ...
    public function getFilterBlock(
        ...
        $filters = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
        ...
        foreach ($filters as $filter) {
            ...
            if ($latest_selected_filters == $selected_filters && isset($productCache[$cacheKey])) {
                ...
                if (!empty($sql_query['from'])) {
                    ...
                    $products = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS($assembled_sql_query);
                }
                ...
                case 'price':
                    if ($this->showPriceFilter()) {
                        ...
                        if ($compute_range_filters && isset($products) && $products) {
                            $rangeAggregator = new FacetedSearchRangeAggregator();
    ...
    private static function getConditionFilterSubQuery($filter_value, $ignore_join = false)
    {
        ...
        foreach ($filter_value as $cond) {
            $query_filters .= '\''.Database::getInstance()->escape($cond).'\',';
        }
    ...
    public function rebuildLayeredCache($products_ids = array(), $categories_ids = array(), $rebuildLayeredCategories = true)
    {
        ...
        $db = Database::getInstance(_PS_USE_SQL_SLAVE_);
        $n_categories = array();
        ...
        if ($to_insert) {
            Database::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'layered_filter(name, filters, n_categories, date_add)
            ...
            $last_id = Database::getInstance()->Insert_ID();
            Database::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'layered_filter_shop WHERE `id_layered_filter` = '.$last_id);
            foreach ($shop_list as $id_shop) {
                Database::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'layered_filter_shop (`id_layered_filter`, `id_shop`)
    ...
    public function buildLayeredCategories()
    {
        ...
        $res = Database::getInstance()->executeS('SELECT * FROM '._DB_PREFIX_.'layered_filter ORDER BY date_add DESC');
        $categories = array();
        // Remove all from layered_category
        Database::getInstance()->execute('TRUNCATE '._DB_PREFIX_.'layered_category');
                        ...
                        foreach ($data as $key => $value) {
                            if (substr($key, 0, 17) == 'layered_selection') {
                                ...
                                if ($nbSqlValuesToInsert >= 100) {
                                    Database::getInstance()->execute($sqlInsertPrefix.rtrim($sqlInsert, ','));
                                    $sqlInsert = '';
                                    $nbSqlValuesToInsert = 0;
                                }
                            }
