...
class Ps_Customtext extends Module implements WidgetInterface
{
    ...
    public function uninstallDB($drop_table = true)
    {
        $ret = true;
        if ($drop_table) {
            $ret &= Database::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'info`')
                && Database::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'info_shop`')
                && Database::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'info_lang`');
        }
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        return array(
            'cms_infos' => Database::getInstance()->getRow($sql),
        );
    }
