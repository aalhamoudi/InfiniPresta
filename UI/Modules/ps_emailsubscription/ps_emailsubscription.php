...
class Ps_Emailsubscription extends Module implements WidgetInterface
{
    ...
    {
        Database::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'emailsubscription');
    ...
    public function isNewsletterRegistered($customer_email)
    {
        ...
        if (Database::getInstance()->getRow($sql)) {
        ...
        if (!$registered = Database::getInstance()->getRow($sql)) {
    ...
    public function getSubscribers()
    {
        ...
        $customers = Database::getInstance(_PS_USE_SQL_SLAVE_)->executeS($dbquery->build());
        ...
        $non_customers = Database::getInstance()->executeS($dbquery->build());
    ...
    protected function unregister($email, $register_status)
    {
        ...
        if (!isset($sql) || !Database::getInstance()->execute($sql)) {
            return false;
        }
    ...
    protected function getGuestEmailByToken($token)
    {
        ...
        return Database::getInstance()->getValue($sql);
    }
    ...
    public function hookActionCustomerAccountAdd($params)
    {
        ...
        if (Validate::isEmail($email)) {
            return (bool) Database::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'emailsubscription WHERE id_shop='.(int) $id_shop.' AND email=\''.pSQL($email)."'");
        }
    ...
    {
        $newsletter = Database::getInstance(_PS_USE_SQL_SLAVE_)->getValue('SELECT `newsletter`
            FROM ' . _DB_PREFIX_ . 'customer
    ...
    public function hookActionExportGDPRData($customer)
    {
        if (!Tools::isEmpty($customer['email']) && Validate::isEmail($customer['email'])) {
            ...
            if ($res = Database::getInstance()->ExecuteS($sql)) {
