...
class Ps_LegalCompliance extends Module
{
    ...
    public function removeCMSPagesIfNeeded()
    {
        if (Module::isInstalled('ps_linklist')) {
            ...
            $link_blocks = Database::getInstance()->executeS($sql);
            foreach ($link_blocks as $link_block) {
                ...
                if ($conditions_found) {
                    ...
                    Database::getInstance()->update('link_block', array('content' => pSQL($content)), '`id_link_block` = '.(int) $link_block['id_link_block']);
                }
            }
        }
    ...
    public function unloadTables()
    {
        ...
        foreach ($sql as $name => $v) {
            $state &= Database::getInstance()->execute('DROP TABLE IF EXISTS '.$name);
        }
    ...
    public function loadTables()
    {
        ...
        $sql = require dirname(__FILE__).'/install/sql_install.php';
        foreach ($sql as $s) {
            $state &= Database::getInstance()->execute($s);
        }
