...
class LinkBlockRepository
{
    ...
    public function __construct(Database $db, Shop $shop, Translator $translator)
    {
        $this->db = $db;
        $this->shop = $shop;
    ...
    public function createOrUpdateLinkList(&$id_link_block, $id_hook, $content, $custom_content)
    {
        ...
        if (empty($id_link_block)) {
            ...
            $success &= Database::getInstance()->execute($query);
            $id_link_block = (int) Database::getInstance()->Insert_ID();
            ...
            if (!empty($success) && !empty($id_link_block)) {
                ...
                if (!empty($languages)) {
                    ...
                    $success &= Database::getInstance()->execute(rtrim($query, ','));
                }
            }
        } else {
