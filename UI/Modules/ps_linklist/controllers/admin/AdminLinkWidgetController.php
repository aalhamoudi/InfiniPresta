...
class AdminLinkWidgetController extends ModuleAdminController
{
    ...
    private function deleteLinkList()
    {
        ...
        if (!empty($id_link_block)) {
            $success &= Database::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'link_block` WHERE `id_link_block` = '.$id_link_block);

            if ($success) {
                $success &= Database::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'link_block_lang` WHERE `id_link_block` = '.$id_link_block);
            }
        }
