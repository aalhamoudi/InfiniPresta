...
class MainMenu extends Module implements WidgetInterface
{
    ...
    public function installDb()
    {
		...
		CREATE TABLE IF NOT EXISTS `linksmenutop` (
            ...
            Database::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `linksmenutop_lang` (
    ...
    {
        Database::getInstance()->execute('DROP TABLE `linksmenutop`');
        Database::getInstance()->execute('DROP TABLE `linksmenutop_lang`');
        return true;
    }
        ...
        if (Tools::isSubmit('submitBlocktopmenu')) {
            ...
            if (Tools::isSubmit('submitBlocktopmenuLinks')) {
                ...
                if (Tools::isSubmit('updatelink')) {
                    ...
                    MenuTopLinks::update($link, $label, $new_window, (int)$id_shop, (int)$id_linksmenutop);
    ...
    protected function getCMSCategories($recursive = false, $parent = 1, $id_lang = false, $id_shop = false)
    {
        ...
        if ($recursive === false) {
            ...
            return Database::getInstance()->executeS($sql);
        } else {
            $sql = 'SELECT bcp.`id_cms_category`, bcp.`id_parent`, bcp.`level_depth`, bcp.`active`, bcp.`position`, cl.`name`, cl.`link_rewrite`
				FROM `'._DB_PREFIX_.'cms_category` bcp'.
                $join_shop.'
            ...
            $results = Database::getInstance()->executeS($sql);
    ...
    {
        $linksmenutop = Database::getInstance()->executeS('
			SELECT *
        ...
        foreach ($linksmenutop as $id => $link) {
            ...
            $linksmenutop[$id]['new_id_linksmenutop'] = Database::getInstance()->Insert_ID();
        }

        foreach ($linksmenutop as $id => $link) {
            $lang = Database::getInstance()->executeS('
    ...
    public function customGetNestedCategories($shop_id, $root_category = null, $id_lang = false, $active = false, $groups = null, $use_shop_restriction = true, $sql_filter = '', $sql_sort = '', $sql_limit = '')
    {
        ...
        if (!Cache::isStored($cache_id)) {
            $result = Database::getInstance()->executeS('
							SELECT c.*, cl.*
    ...
    public function renderWidget($hookName, array $configuration)
    {
        ...
        return $this->fetch('module:mainmenu/mainmenu.tpl');
    }
}
